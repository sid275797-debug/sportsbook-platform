...
const STATUS_FILTER = ['all', 'accepted', 'settled_win', 'settled_loss', 'cancelled', 'void']
...
export default function MyBets() {
  ...
  const stats = {
    total:   bets.length,
    won:     bets.filter(b => b.status === 'settled_win').length,
    pending: bets.filter(b => b.status === 'accepted' || b.status === 'pending').length,
    staked:  bets.reduce((s, b) => s + (b.totalStake ?? 0), 0),
    returns: bets.filter(b => b.status === 'settled_win').reduce((s, b) => s + (b.potentialPayout ?? 0), 0),
  }
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
          {STATUS_FILTER.map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{
              ...
              {STATUS_LABELS[f]} {f !== 'all' && `(${bets.filter(b => b.status === f).length})`}
            </button>
          ))}
        </div>
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { bg: string; color: string; label: string }> = {
    settled_win:  { bg: 'rgba(34,197,94,0.12)',   color: '#22c55e', label: 'WON' },
    settled_loss: { bg: 'rgba(224,63,63,0.12)',    color: '#e03f3f', label: 'LOST' },
    accepted:     { bg: 'rgba(240,165,0,0.12)',    color: '#f0a500', label: 'PENDING' },
    pending:      { bg: 'rgba(240,165,0,0.12)',    color: '#f0a500', label: 'PENDING' },
    cancelled:    { bg: 'rgba(255,255,255,0.06)',  color: 'var(--text-muted)', label: 'VOID' },
    void:         { bg: 'rgba(255,255,255,0.06)',  color: 'var(--text-muted)', label: 'VOID' },
  }
  const s = map[status] ?? { bg: 'var(--bg-elevated)', color: 'var(--text-muted)', label: status.toUpperCase() }
  return (
