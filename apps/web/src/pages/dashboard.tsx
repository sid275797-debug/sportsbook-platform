...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      walletApi.balance().then(r => setBalance(r.data.data?.available ?? 0)),
  ...
  const wonBets   = bets.filter(b => b.status === 'settled_win').length
  const lostBets  = bets.filter(b => b.status === 'settled_loss').length
  const pendBets  = bets.filter(b => b.status === 'accepted' || b.status === 'pending').length
  const totalStaked = bets.reduce((s, b) => s + (b.totalStake ?? 0), 0)
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { bg: string; color: string }> = {
    settled_win:  { bg: 'rgba(34,197,94,0.1)',   color: '#22c55e' },
    settled_loss: { bg: 'rgba(224,63,63,0.1)',    color: '#e03f3f' },
    accepted:     { bg: 'rgba(240,165,0,0.1)',    color: '#f0a500' },
