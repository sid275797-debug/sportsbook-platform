...
export class AuthController {
  async register(req: FastifyRequest, reply: FastifyReply) {
    ...
    } catch (err) { log.warn({ err }, 'Failed to publish user registered event') }

    log.info({ userId: user.id }, 'User registered')
    return reply.status(201).send({ success: true, data: { accessToken, refreshToken, user: { id: user.id, email: user.email, username: user.username, role: user.role } } })
  }
  ...
  async refreshToken(req: FastifyRequest, reply: FastifyReply) {
    ...
    try {
      ...
      await db.session.update({ where: { id: session.id }, data: { refreshToken: newRefreshToken, expiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) } })
  ...
  async sendOtp(req: FastifyRequest, reply: FastifyReply) {
    ...
    log.info({ phone }, `OTP generated (in prod send via SMS): ${otp}`)
  ...
  async me(req: FastifyRequest, reply: FastifyReply) {
    ...
    const user = await db.user.findUnique({ where: { id: req.user!.userId }, select: { id: true, email: true, username: true, phone: true, role: true, kycStatus: true, createdAt: true } })
