...
const placeBetSchema = z.object({
  selections: z.array(z.object({
    marketId: z.string().uuid(), outcomeId: z.string().uuid(),
    odds: z.number().positive(), stake: z.number().positive().min(10),
  })).min(1).max(20),
  ...
  app.post('/place', async (req, reply) => {
    ...
    try {
      const betSlip = await bettingService.placeBet({
        ...result.data, currency: result.data.currency as Currency, userId: req.user!.userId,
      })
