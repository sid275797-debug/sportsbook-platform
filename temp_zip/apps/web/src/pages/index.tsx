...
const HERO_BANNERS = [
  { label: 'IPL 2025', sub: 'Live & Upcoming matches', color: '#f0a500', emoji: '🏏' },
  { label: 'UEFA Champions League', sub: 'Quarter Finals • Live Now', color: '#00d4aa', emoji: '⚽' },
  { label: 'Casino Crash', sub: 'Up to 1000x multiplier', color: '#e03f3f', emoji: '🚀' },
]
    ...
    <Layout>
      ...
      <div style={{ padding: '0' }} className="fade-up">
        ...
        <div style={{
          ...
          {['All', '⚽ Football', '🏏 Cricket', '🎾 Tennis', '🏀 Basketball', '🏒 Hockey', '⚾ Baseball', '🎰 Casino'].map((label) => (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8 }}>
              {[
                ...
                <Link key={g.href} href={g.href} style={{
                  ...
                  <span style={{ fontSize: 11, color: g.color }}>{g.desc}</span>
                </Link>
              ))}
            </div>
          </section>
...
export const getServerSideProps: GetServerSideProps = async () => {
  ...
  try {
    const [liveRes, upcomingRes] = await Promise.allSettled([
      ...
      axios.get(`${API}/api/fixtures/upcoming?limit=12`),
    ])
    return {
      props: {
