...
const NAV_SPORTS = [
  ...
  { icon: '🏈', label: 'NFL',            slug: 'american-football' },
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', gap: 0, flex: 1, overflowX: 'auto' }}>
          {[
            ...
            { href: '/sportsbook?sport=cricket', label: '🏏 Cricket' },
            { href: '/casino',     label: '🎰 Casino' },
            { href: '/promotions', label: '🎁 Promos' },
          ].map(({ href, label }) => {
            const active = router.pathname === href || (href !== '/' && router.asPath.startsWith(href.split('?')[0]))
            return (
        ...
        {!hideSidebar && (
          <aside style={{
            ...
            <div style={{ padding: '10px 0' }}>
              <div style={{ padding: '6px 16px', fontSize: 10, fontWeight: 700, letterSpacing: 1.2, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Sports</div>
              {NAV_SPORTS.map(s => (
